...
enum class JointRole : unsigned int
{
	...
	BendingPower = 1 << 0,///< Role for the joint that set the recoil of bodies that bend a bow.
};
...
inline bool jointDefHasRole(b2JointDef* jointDef, JointRole role)
{
	return jointDef->userData and (reinterpret_cast<unsigned int>(jointDef->userData) & static_cast<unsigned int>(role));
}
